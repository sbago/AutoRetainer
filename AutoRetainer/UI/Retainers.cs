...
internal unsafe class Retainers
{
    ...
    internal static void Draw()
    {
        ...
        {
            ImGuiEx.Text(Language.Language.Instance.Text_DataNotReady);
            return;
        }

        var slots = Utils.GetInventoryFreeSlotCount();
        var ventures = InventoryManager.Instance()->GetInventoryItemCount(21072);
        ImGuiEx.Text(Language.Language.Instance.Text_InventorySlots);
        ImGui.SameLine(0, 0);
        ...
        ImGui.SameLine();
        ImGuiEx.Text(Language.Language.Instance.Text_Ventures);
        ImGui.SameLine(0, 0);
        ImGuiEx.Text(ventures < 2 * P.retainerManager.Count ? ImGuiColors.DalamudRed : ventures < 24 * P.retainerManager.Count ? ImGuiColors.DalamudOrange : ImGuiColors.ParsedGreen,
            $"{ventures}");
        ImGuiComponents.HelpMarker(Language.Language.Instance.HelpMarker_Ventures);
        ...
        ImGui.TableSetupColumn(Language.Language.Instance.TableSetupColumn_Name, ImGuiTableColumnFlags.WidthStretch);
        ImGui.TableSetupColumn(Language.Language.Instance.TableSetupColumn_Venture);
        ImGui.TableSetupColumn(Language.Language.Instance.TableSetupColumn_Interaction);
        ...
        ImGui.EndTable();
        ImGuiEx.ImGuiLineCentered(Language.Language.Instance.ImGuiLineCentered_Timeouts, delegate
        {
            if (ImGui.SmallButton(Language.Language.Instance.SmallButton_Timeouts))
            {
